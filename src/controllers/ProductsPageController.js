  ...
  try {
    ...
    if (brand) {
      ...
      if (foundBrands.length !== brandNames.length) {
        throw new ValidationError("One or more selected brands are invalid.")
      }
    ...
    if (type) {
      ...
      if (foundTypes.length !== typeNames.length) {
        throw new ValidationError("One or more selected types are invalid.")
      }
    ...
    if (page > 1 && page > totalPages) {
      throw new ValidationError(`Page ${page} is out of range. Max page is ${totalPages || 1}.`)
    }
...
const getProductFilterList = async (req, res) => {
  return succeed(res, 200, { 'brand': (await ProductBrand.find({}, 'name')).map((e) => e.name), 'type': (await ProductType.find({}, 'name')).map((e) => e.name) })
}
